...
namespace ProxyBase
{
  public class Wayregion : Form
  {
    ...
    {
      Npc npc = new Npc();
      ushort num1 = (ushort) (e.X / this.matrix1.MaxtrixSize);
      ushort num2 = (ushort) (e.Y / this.matrix1.MaxtrixSize);
      Location key = new Location((int) num1, (int) num2);
      ...
      else if (this.addwaypoints.Checked)
      {
        ...
        npc.Location.X = (int) num1;
        npc.Location.Y = (int) num2;
    ...
    private void SaveMapPack()
    {
      ...
      xdocument.Element((XName) "WayPoints").Add((object) new XElement((XName) "NumberOfMaps", (object) num1));
      int num2 = 0;
      foreach (KeyValuePair<int, MapNum> tempRegion in this.Client.TempRegions)
      {
        if (tempRegion.Value.Regions.Count > 0)
        {
          xdocument.Element((XName) "WayPoints").Add((object) new XElement((XName) ("map" + (object) num2)));
          xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) num2)).Add((object) new XElement((XName) "Count", (object) tempRegion.Value.Regions.Count.ToString()));
          xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) num2)).Add((object) new XElement((XName) "MapNum", (object) tempRegion.Key.ToString()));
          int num3 = 0;
          foreach (KeyValuePair<Location, string> region in tempRegion.Value.Regions)
          {
            if (region.Key != null)
            {
              XElement xelement1 = xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) num2));
              XName name = (XName) ("Location" + (object) num3);
              int num4 = region.Key.X;
              string str1 = num4.ToString();
              num4 = region.Key.Y;
              string str2 = num4.ToString();
              string str3 = str1 + "," + str2;
              XElement xelement2 = new XElement(name, (object) str3);
              xelement1.Add((object) xelement2);
              xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) num2)).Add((object) new XElement((XName) ("Point" + (object) num3), (object) region.Value.ToString()));
              ++num3;
            }
          }
          ++num2;
    ...
    public void LoadMapPack(string FileName = "")
    {
      ...
      for (int index1 = 0; index1 < (int) Convert.ToUInt16(xdocument.Element((XName) "WayPoints").Element((XName) "NumberOfMaps").Value); ++index1)
      {
        ...
        mapNum.Number = (int) Convert.ToUInt16(xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) index1)).Element((XName) "MapNum").Value);
        this.Client.TempRegions.Add(mapNum.Number, mapNum);
        for (int index2 = 0; index2 < (int) Convert.ToUInt16(xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) index1)).Element((XName) "Count").Value); ++index2)
        {
          Npc npc = new Npc();
          string[] strArray = xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) index1)).Element((XName) ("Location" + (object) index2)).Value.Split(',');
          int x = int.Parse(strArray[0]);
          int y = int.Parse(strArray[1]);
          Location key = new Location(x, y);
          string str = xdocument.Element((XName) "WayPoints").Element((XName) ("map" + (object) index1)).Element((XName) ("Point" + (object) index2)).Value;
